      ...
      unsubscribe = thingsRef
        ...
        .onSnapshot((querySnapshot) => {
          window.location.href = querySnapshot.empty
            ? "/pages/preferences/PreferencesLoad.html"
            : "/pages/exercises/ELoad.html";
        });
    } else {
...
async function getUserPreferences(userInfo, allPreferences, apiKey) {
  ...
  try {
    ...
    let selectedPreferences = aiResponse
      .split(",")
      .map((pref) => pref.trim())
      .filter((pref) => pref !== "" && allPreferences.includes(pref));
    ...
    if (user) {
      ...
      unsubscribe = thingsRef
        ...
        .onSnapshot((querySnapshot) => {
          ...
          window.location.href = querySnapshot.empty
            ? "/pages/routine/RoutineGenForm.html"
            : "/pages/routine/RoutineList.html";
        });
    } else {
      unsubscribe && unsubscribe();
    }
